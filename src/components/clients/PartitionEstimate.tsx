  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(estimateRef, (doc) => {
      if (doc.exists()) {
        ...
        const firstFloorHeight = data.firstFloorHeight || '2,5 метра';
        const secondFloorHeight = data.secondFloorHeight || '2,5 метра';

        // Convert heights to numbers
        const firstHeight = parseFloat(firstFloorHeight.split(' ')[0].replace(',', '.'));
        const secondHeight = parseFloat(secondFloorHeight.split(' ')[0].replace(',', '.'));
        
        setEstimateData(prev => {
          const newItems = [...prev.items];

          // Гипсокартон влагостойкий
          const moistureResistantIndex = newItems.findIndex(item => 
            item.name === 'Гипсокартон 12,5мм влагостойкий стеновой (Для межком перег) пр-ва Knauf'
          );
          if (moistureResistantIndex !== -1) {
            const quantity = Math.ceil(((firstFloorWallsLength * firstHeight) + (secondFloorWallsLength * secondHeight)) / 3);
            newItems[moistureResistantIndex] = {
              ...newItems[moistureResistantIndex],
              quantity,
              total: quantity * newItems[moistureResistantIndex].price
            };
          }

          // Гипсокартон обычный
          const regularIndex = newItems.findIndex(item => 
            item.name === 'Гипсокартон 12,5мм стеновой (Для межкомнатных перегородок) пр-ва Knauf'
          );
          if (regularIndex !== -1 && moistureResistantIndex !== -1) {
            const quantity = Math.ceil(((gklWallsArea * 2) / 3) + 2) - newItems[moistureResistantIndex].quantity;
            newItems[regularIndex] = {
              ...newItems[regularIndex],
              quantity,
              total: quantity * newItems[regularIndex].price
            };
          }
          ...
          if (profileIndex !== -1) {
            const quantity = Math.ceil(gklWallsLength / 0.5);
          ...
          if (mineralWoolIndex !== -1) {
            const quantity = Math.ceil(gklWallsArea / 7.51);
          ...
          if (screwsIndex !== -1 && regularIndex !== -1) {
            const quantity = Math.ceil(newItems[regularIndex].quantity / 5);
