    ...
    const unsubscribe = onSnapshot(estimateRef, (doc) => {
      if (doc.exists()) {
        ...
        setEstimateData(prev => {
          ...
          // Обновляем зависимые позиции
          newItems.forEach((item, index) => {
            if (item.name === 'Гвозди 120') {
              const brusQuantity = newItems[brusIndex].quantity;
              const quantity = Math.ceil(brusQuantity * 0.15);
              newItems[index] = {
                ...item,
                quantity,
                total: quantity * item.price
              };
            } else if (item.name === 'Гвозди 70 (Для монтажа обрешетки)') {
              const brusQuantity = newItems[brusIndex].quantity;
              const quantity = Math.ceil(brusQuantity * 0.166);
              newItems[index] = {
                ...item,
                quantity,
                total: quantity * item.price
              };
            } else if (item.name === 'Шурупы 4 (Для монтажа металлочерепицы)') {
              const metalTileQuantity = newItems[metalTileIndex].quantity;
              const quantity = Math.ceil(0.074 * metalTileQuantity);
              newItems[index] = {
                ...item,
                quantity,
                total: quantity * item.price
              ...
              const polystyreneQuantity = newItems[polystyreneIndex].quantity;
              const quantity = Math.ceil(0.5 * polystyreneQuantity);
              newItems[index] = {
                ...item,
                quantity,
                total: quantity * item.price
              };
            } else if (item.name === 'Скобы (Для крепления паро пленки)') {
              const vaporBarrierQuantity = newItems[vaporBarrierIndex].quantity;
              const quantity = Math.ceil(vaporBarrierQuantity / 2);
              newItems[index] = {
                ...item,
                quantity,
                total: quantity * item.price
              ...
              const osbQuantity = newItems[osbIndex].quantity;
              const quantity = Math.ceil(osbQuantity / 5);
              newItems[index] = {
                ...item,
                quantity,
                total: quantity * item.price
